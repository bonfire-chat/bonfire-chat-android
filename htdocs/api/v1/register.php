...
$phone = preparePhonenumber($data["phone"]);
...
if ($stmt->rowCount()) {
    ...
    $ok = $up->execute(array( $data["nickname"], $phone, $data["gcmid"], $_SERVER["REMOTE_ADDR"], $info['id']));
    ...
    $ok=$stmt->execute(array($data["nickname"], $pubkey, $phone, $data["gcmid"], $_SERVER["REMOTE_ADDR"]));
