...
if (!isset($data["xmppid"]) || !isset($data["nickname"]) || !isset($data["phone"]) || !isset($data["gcmid"])) {
...
if ($stmt->rowCount()) {
    ...
    $up = $db->prepare("UPDATE users SET xmppid=?, nickname=?, phone=?, gcmid=?, last_updated=NOW(), ip=? WHERE id =?");
    $ok = $up->execute(array($data["xmppid"], $data["nickname"], $data["phone"], $data["gcmid"], $_SERVER["REMOTE_ADDR"], $info['id']));
